...
class MovableTypeService < DonWebService
...
#  before_invocation :authenticate, :except => [:getTrackbackPings, :supportedMethods, :supportedTextFilters]
  before_invocation :authenticate, :except => [:supportedMethods]

  def getRecentPostTitles(blogid, username, password, numberOfPosts)
    Article.find(:all,:order => "article_mtime DESC", :limit => numberOfPosts).collect do |article|
      MovableTypeStructs::ArticleTitle.new(
            :dateCreated => article.article_mtime,
            :userid      => blogid.to_s,
